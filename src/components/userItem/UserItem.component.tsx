 ...
 */
const UserItem = ({ user, variant = "default", onClick, rightSlot, className }: UserItemProps) => {
          ...
          <div className={styles.nameLine}>
            ...
            {variant === "default" && (
              <div className={styles.chips}>
                <Tag className={styles.chip} color={statusInfo.color} icon={statusInfo.icon}>
                  {statusInfo.text}
                </Tag>

                {primaryProfileRef && (
                  <Tag className={styles.chip} color={getProfileRefColor(primaryProfileRef[0])}>
            ...
            <>
              {user?.permissions && user.permissions.length > 0 ? (
                <Tooltip title={`${user.permissions.length} permissions`}>
                  <div className={styles.permissionCount}>{user.permissions.length}</div>
                </Tooltip>
              ) : (
        ...
        <div className={styles.rowBottom}>
          <div className={styles.permissionPreview}>
            ...
            {user.permissions.length > 3 && (
              <span className={styles.morePermissions}>+{user.permissions.length - 3} more</span>
            )}
          </div>
        </div>
      )}
